...
The macOS app runs a proactive "Argus" layer that continuously captures screenshots every 2.5 seconds and batches them into groups of four for VLM analysis every ~10 seconds. The VLM (Gemini 2.5 Pro) performs diff-based analysis across frames to detect friction patterns -- repetitive window switching, idle stalls, tedious manual data entry -- and returns structured JSON with task state updates, friction classifications, and proposed actions. When friction is detected, an agentic executor (Gemini 3 Flash) runs a multi-step tool-use loop (up to 10 steps with file read/write, command execution, and vision) to actually perform the work for the user, not just suggest it. Screenshots are never persisted or sent to the backend; only the JSON analysis results are synced.

The backend is a Python FastAPI server on a DigitalOcean droplet with PostgreSQL 16, fronted by nginx with SSL. It manages users (Argon2id password hashing + Apple Sign In), tasks with separately-tabled steps (avoiding JSONB read-modify-write bottlenecks for the VLM's ~20-second update cycle), focus sessions with checkpoint context, and distraction event logging. Brain Dump voice input is transcribed on-device via WhisperKit, then parsed by the LLM into structured tasks with auto-generated 5-15 minute ADHD-friendly steps. Cross-device focus sessions use Apple Live Activities and ActivityKit push tokens to keep macOS, iOS, and iPadOS in sync, with Screen Time's DeviceActivityMonitor enforcing distraction thresholds on mobile. Analytics are powered by Hex API notebooks querying Postgres directly for distraction patterns and focus trends.
